...
const login = async (email: string, password: string) => {
  ...
  const logout = () => {
    ...
    if (token) {
      const decodedToken: { id: number; role: number; email: string; is_active :boolean } = jwtDecode(token);
      socket.emit('logout');
    }
  ...
  const redirectToDashboard = (role: number) => {
    switch (role) {
      case 1:
        ...
        if (token) {
          const decodedToken: { id: number; role: number; email: string} = jwtDecode(token);
